 ...
 * 1. 每日热门
 */
public enum PromotionTypeDic implements EnumBase {
     ...
     */
    @JsonCreator
    public static PromotionTypeDic getItem(final int code) {
        final EnumBase result = valueMap.get(code);
        if (result == null) {
            throw new IllegalArgumentException("输入的code" + code + "不在枚举的取值范围内");
        }
        return (PromotionTypeDic) result;
    }
     ...
     * 构造器，传入code
     */
    PromotionTypeDic(final int code) {
        this.code = code;
    }
