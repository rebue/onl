...
public class OnlOnlineSvcImpl extends MybatisBaseSvcImpl<OnlOnlineMo, java.lang.Long, OnlOnlineMapper> implements OnlOnlineSvc {
    ...
    public int add(final OnlOnlineMo mo) {
        _log.info("添加上线信息");
        // 如果id为空那么自动生成分布式id
    ...
    public AddOnlineRo addOnline(final AddOnlineTo to) {
        ...
        for (int i = 0; i < to.getOnlineSpecs().size(); i++) {
            ...
            final BigDecimal cashbackAmount = to.getSubjectType() == 0 ? to.getOnlineSpecs().get(i).getCashbackAmount() : amount;
            ...
            final BigDecimal commissionAmount = to.getSubjectType() == 0 ? amount : to.getOnlineSpecs().get(i).getSalePrice();
    ...
    public ReOnlineRo reOnline(final AddOnlineTo to) {
        ...
        if (to.getOnlineId() == null || to.getOnlineName() == null || to.getOnlineName().equals("") || to.getGoodsQsmm() == null || to.getGoodsQsmm().equals("")
                || to.getOnlineSpecs().size() == 0 || to.getSlideshow().size() == 0 || to.getSupplierId() == null || to.getDeliverOrgId() == null) {
    ...
    public PageInfo<OnlOnlineListRo> listEx(final OnlOnlineListRo ro, final int pageNum, final int pageSize, final String orderBy) {
        ...
        final PageInfo<OnlOnlineMo> onlinePageInfo = PageHelper.startPage(pageNum, pageSize, orderBy).doSelectPageInfo(() -> _mapper.selectSelective(mo));
    ...
    /**
     * 更新上线信息(下单后)
     * 1. 如果上线数量-销售数量<购买数量，返回库存不足
     * 2. 更新上线规格商品的销售数量=原销售数量+购买数量
     * 3. 删除用户的购物车
     * 4. 如果上线的所有规格的上线数量都<=销售数量都，那么此上线记录自动下线
     */
    @Override
    @Transactional(readOnly = false, propagation = Propagation.REQUIRED)
    public Ro updateOnlineAfterOrder(final UpdateOnlineAfterOrderTo to) {
        ...
        for (final UpdateOnlineSpecAfterOrderTo specTo : to.getSpecList()) {
            ...
            final int rowCount = onlOnlineSpecSvc.updateSaleCount(specTo.getBuyCount(), specTo.getOnlineSpecId(), onlineSpecMo.getSaleCount());
